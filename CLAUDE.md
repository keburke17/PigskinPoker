...
2. `npm test` before committing: **400 passed, 1 skipped, 22 files**. If the output says
...
401 tests. Three groups worth knowing about:
...
- **`rls.test.js`, `server.test.js`, `bootstrap.test.js`** need the local Supabase stack
  (started for you by `npm run dev`) and **skip themselves silently without it** - 142 of the 400
