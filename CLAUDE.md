...
2. `npm test` before committing: **426 passed, 1 skipped, 24 files**. If the output says
...
427 tests. Three groups worth knowing about:
...
- **`rls.test.js`, `server.test.js`, `bootstrap.test.js`** need the local Supabase stack
  (started for you by `npm run dev`) and **skip themselves silently without it** - 142 of the 426
