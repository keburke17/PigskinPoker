...
2. `npm test` before committing: **334 passed, 1 skipped, 20 files**. If the output says
...
334 tests. Three groups worth knowing about:
...
- **`rls.test.js`, `server.test.js`, `bootstrap.test.js`** need the local Supabase stack
  (started for you by `npm run dev`) and **skip themselves silently without it** - 120 of the 334
...
| **Live stats feed** | **Half done.** The pool refreshes from nflverse depth charts, scoring splits into passing / rushing / receiving (Phase 4 stages 1 and 4, live since 2026-08-29), and each period now carries the NFL week it plays (stage 3, `server/schedule.js`). **The weekly stats pull is not built** - stats are still typed in. Stages 5-7 in `docs/PHASE-4-PLAN.md`; `docs/LIVE-DATA.md` is the provider survey behind the choice. |
